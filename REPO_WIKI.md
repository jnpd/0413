...
12. `docs/04-data-model/README.md`
13. `docs/04-data-model/核心实体说明.md`
14. `docs/02-business-rules/业务规则选路索引.md`
15. `docs/02-business-rules/接口规范.md`
16. `docs/02-business-rules/多租户隔离规则.md`
17. `docs/02-business-rules/权限边界规则.md`
...
- 数据模型：`docs/04-data-model/核心实体说明.md`

### 高风险知识

- 规则选路入口：`docs/02-business-rules/业务规则选路索引.md`
- 多租户隔离：`docs/02-business-rules/多租户隔离规则.md`
- 权限边界：`docs/02-business-rules/权限边界规则.md`
- 批次快照检验台：独立规则页待补，当前先参考 `AGENTS.md` 与 `docs/00-meta/缺口台账.md`
- 状态一致性：`docs/02-business-rules/状态一致性规则.md`
- 枚举与字典：`docs/02-business-rules/枚举与字典代码落地规范.md`
...
- BladeX 前端登录与仓库接口联调说明：`docs/02-business-rules/BladeX前端登录与仓库接口联调说明.md`
- 仓库页本地联调前置清单：`docs/06-release-ops/仓库页本地联调前置清单.md`

## 维护规则

- 新需求先进入 `changes/<YYYY-MM>/<feature>/proposal.md`
- 当前仓库内，需求确认后的 spec / design / tasks / test-cases / acceptance / contracts 仍维护在 `openspec/changes/<feature>/`
- 若后续完成 starter 迁移，再切换到 `specs/<YYYY-MM>/<feature>/`
- 接口变更后同步更新 `docs/02-business-rules/接口规范.md` 或 feature `contracts/`
- 高风险规则变更后同步更新知识页
- 线上问题修复后同步回灌缺陷案例
